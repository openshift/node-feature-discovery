...
func argsParse(argv []string) (args Args) {
  ...
  --label-whitelist=<pattern> Regular expression to filter label names to
	...
	if err != nil {
		stderrLogger.Fatalf("invalid --port defined: %s", err)
	}
	...
	if args.certFile != "" || args.keyFile != "" {
		if args.certFile == "" {
			stderrLogger.Fatalf("ERROR: --cert-file needs to be specified alongside --key-file")
		}
		if args.keyFile == "" {
			stderrLogger.Fatalf("ERROR: --key-file needs to be specified alongside --cert-file")
		}
	}
	return args
}
...
func (s *labelerServer) SetLabels(c context.Context, r *pb.SetLabelsRequest) (*pb.SetLabelsReply, error) {
	...
	if !s.args.noPublish {
		...
		sort.Strings(keys)
		annotations := Annotations{"slave.version": r.NfdVersion,
